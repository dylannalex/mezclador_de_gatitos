...
# Cargar todas las imágenes de gatos desde el directorio 'data/cat'
def load_images():
    """Cargar rutas de imágenes de gatos."""
    cat_images = os.listdir(os.path.join("data", "cat"))
    return [load_image(os.path.join("data", "cat", img)) for img in cat_images]
...
def load_model():
    ...
    vae.load_state_dict(torch.load(os.path.join("models", "vae.pt"), map_location=torch.device('cpu')))
    return vae

# Realizar interpolación lineal entre dos imágenes en el espacio latente
def linear_interpolation(model: VAE, img_1: Image.Image, img_2: Image.Image, device: str, steps: int = 10):
    ...
    with torch.no_grad():
        ...
        interps = np.array([np.linspace(v1[i], v2[i], steps+2) for i in range(v1.shape[0])]).T
        interps = torch.tensor(interps).to(device).squeeze()
        interps = interps.permute(1, 0)
        decoded_interps = model.decode(interps.to(device))

    decoded_interps = [img.squeeze().permute(1, 2, 0).cpu().numpy() for img in decoded_interps]
...
def tsne_visualization(tsne_results, selected_indexes):
    ...
    scatter_chart = alt.Chart(tsne_df).mark_circle().encode(
